...
export default function App() {
  ...
  const session = useQuery(api.prompts.getSession, 
    sessionId ? { sessionId } : "skip"
  );
  const questions = useQuery(api.prompts.getQuestions,
    sessionId ? { sessionId } : "skip"
  ) ?? [];
              ...
              {session?.status === "complete" && session.enhancedPrompt && (
                <div className="space-y-8">
                  <div className="bg-gradient-to-br from-gray-50 to-gray-100/50 p-6 rounded-xl border border-gray-200/50 shadow-sm">
                    <h3 className="text-lg font-semibold text-gray-800 mb-3">Enhanced Prompt</h3>
